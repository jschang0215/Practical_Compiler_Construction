...
// Extracts next character from input stream
...
// Return value of hexadecimal literals
static int hexchar(void)
{
    int c, h, n = 0, f = 0;
    while (isxdigit(c = next()))
    {
        h = chrpos("0123456789abcdef", tolower(c)); // get value
        n = n * 16 + h;
        f = 1;
    }
    putback(c);
...
// Extracts next character of a string or character literals
...
// Scans decimal, octal, and hexadecimal integers and returns their value
static int scanint(int c)
{
    int val = 0, radix, k, i = 0;
    radix = 10;
    if ('0' == c)
    {
        Text[i++] = '0';
...
int skip(void)
{
    ...
    for (;;)
    {
        ...
        // Invoke preprocessor when # after newline
        if (nl && c == '#')
        {
            preproc();
            c = next();
...
// Checks whether `s` is SubC keyword
...
// Checks whether name exists and refers to a macro
...
// Principal interface of SubC scanner
int scan(void)
{
    int t;
    do
    ...
    } while (frozen(1)); // condition of current iffder is not satisfied
    if (t == Syntoken)
        Syntoken = 0;
    return t;
}

// Does not expand macros and ignores #ifdef context
int scanraw(void)
{
    int t, oisp;
...
// Puts current token and its attributes to *reject queue*
void reject(void)
{
    Rejected = Token;
    Rejval = Value;
