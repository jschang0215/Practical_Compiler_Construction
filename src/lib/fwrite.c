...
int _fsync(FILE *f) {
	...
	if (_file_end[fi] >= _file_size[fi]) {
		...
		k = write(_file_fd[fi], _file_buf[fi] + ptr, end-ptr);
		if (k != end-ptr) {
		...
		for (p = ptr; p < end; p++)
			if ('\n' == buf[p]) pn = p+1;
		if (pn >= 0) {
			k = write(_file_fd[fi], _file_buf[fi] + ptr, pn-ptr);
			_file_ptr[fi] = pn;
			if (k != pn - ptr) {
				_file_iom[fi] |= _FERROR;
				errno = EIO;
...
int _fwrite(void *p, int size, FILE *f) {
	...
	if ((_file_mode[fi] & _IOACC) == _IONBF) {
		if ((k = write(_file_fd[fi], p, size)) != size) {
			_file_iom[fi] |= _FERROR;
			errno = EIO;
		}
		return k;
	...
	while (size > len) {
		if ((k = write(_file_fd[fi], p, len)) != len) {
			_file_iom[fi] |= _FERROR;
			errno = EIO;
			return total - size;
		}
